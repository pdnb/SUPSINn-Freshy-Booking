...
## Domain
Mutations go through application services (`OrderService`, `CatalogService`, `BookingRoundService`, `ShippingRateService`, `AdsBannerService`, `ProductionSummaryService`, `InventoryService`). Inventory is on-hand vs confirmed orders and must not block checkout or change catalog SKU/stock rules.
...
## Auth
Staff login at `/admin/login`. Keep `AdminUserSeeder` and auth redirects. Login may keep the storefront layout; the signed-in console uses `layouts.admin`.
...
The สรุป panel shows the full guest block (ผู้จอง, รหัสนักศึกษา, คณะ, โทร, วิธีรับ, ที่อยู่ with `white-space: pre-line`), line items with qty + line price, and totals (ยอดสินค้า / ค่าส่ง · rate name / ยอดสุทธิ). Do not collapse guest fields into a single paragraph.
